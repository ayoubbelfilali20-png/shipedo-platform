...
export default function DeliverySettingsPage() {
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem('shipedo_delivery')
      if (stored) {
    ...
    <div className="p-6 space-y-4 max-w-md">
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
        <div className="flex items-center gap-4 mb-4">
          <div className="w-14 h-14 rounded-full bg-emerald-100 flex items-center justify-center text-xl font-bold text-emerald-600">
            {(name || '?')[0]}
          </div>
          <div>
      ...
      <button onClick={() => { clearUser('delivery'); router.push('/login') }}
