...
import { Package, CheckCircle, RotateCcw, Phone, MapPin, Calendar } from 'lucide-react'
...
export default function DeliveryDashboard() {
  ...
  useEffect(() => {
    supabase.from('orders')
      .select('id, tracking_number, delivery_tracking, customer_name, customer_phone, customer_city, customer_address, status, total_amount, shipped_to_agent_at')
  ...
  const toDeliver = useMemo(() => orders.filter(o => o.status === 'shipped_to_agent'), [orders])
  const delivered = useMemo(() => orders.filter(o => o.status === 'delivered').length, [orders])
  const returned = useMemo(() => orders.filter(o => o.status === 'returned').length, [orders])
    ...
    <div className="p-6 space-y-6">
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
          ...
          <p className="text-2xl font-bold text-purple-600">{toDeliver.length}</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
          <div className="flex items-center gap-2 mb-2">
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
          ...
          <p className="text-2xl font-bold text-red-600">{returned}</p>
        </div>
      </div>

      <div className="bg-white rounded-xl border border-gray-100 shadow-sm">
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
          <h2 className="font-bold text-[#1a1c3a]">Orders to Deliver ({toDeliver.length})</h2>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-1 mt-0.5 text-[10px] text-gray-400">
                  ...
                  {o.shipped_to_agent_at && (
                    <span className="ml-2"><Calendar size={10} className="inline" /> {new Date(o.shipped_to_agent_at).toLocaleDateString('en-GB', { day: '2-digit', month: 'short' })}</span>
                  )}
                </div>
              </div>
