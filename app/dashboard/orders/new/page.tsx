...
// Map country → available product origins
...
function ProductSelector({ countryCode, selected, onAdd }: {
  ...
  const available = mockProducts.filter(p =>
      ...
      {open && (
        <div className="absolute left-0 right-0 top-full mt-1 bg-white border border-gray-200 rounded-2xl shadow-2xl z-30 overflow-hidden">
          ...
          <div className="max-h-56 overflow-y-auto">
            {available.length === 0 ? (
              <div className="py-8 text-center text-xs text-gray-400">No products available for this country</div>
            ) : available.map(p => {
...
export default function NewOrderPage() {
  ...
  const handleCountryChange = (code: string) => {
    setCountry(code)
    setCity('')
    setRows([]) // reset products when country changes
  }
  ...
  if (generatedId) {
    return (
      <div className="min-h-screen bg-[#f8fafc]">
        <Header title="New Order" subtitle="Create a new customer order" role="admin" />
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc]">
      ...
      <div className="p-6 max-w-2xl">
        <Link href="/dashboard/orders" className="inline-flex items-center gap-1.5 text-sm text-gray-400 hover:text-[#1a1c3a] mb-5 transition-colors">
        ...
        <div className="space-y-4">
              ...
              {rows.length > 0 && (
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-[1fr_80px_110px_32px] gap-2 px-1">
                    <div className="text-[10px] font-bold text-gray-400 uppercase tracking-wide">Product</div>
                    <div className="text-[10px] font-bold text-gray-400 uppercase tracking-wide">Qty</div>
                    <div className="text-[10px] font-bold text-gray-400 uppercase tracking-wide">Price (KES)</div>
                    <div />
                  </div>
                  {rows.map(r => {
          ...
          <div className="flex items-center justify-between pb-6">
            <Link href="/dashboard/orders" className="flex items-center gap-2 px-5 py-2.5 border border-gray-200 text-gray-500 text-sm font-medium rounded-xl hover:bg-gray-50 transition-all">
